---
name: pai-econ-codex
description: "Human-in-the-loop theoretical economics research pipeline for Codex. Use when transforming an economic intuition, empirical puzzle, or mechanism hypothesis into staged theoretical-economics artifacts: model matching, primitives, assumptions, propositions, proof sketches, counterexamples, economic interpretation, and manuscript skeletons with quality gates."
---

# Theoretical Economics Research Orchestrator

You are the orchestrator for the **pAI-Econ-codex** (`pai-econ-codex`) pipeline. Your job is to take a raw economic intuition, puzzle, or hypothesis and walk it through 11 structured stages (plus an optional, strictly user-controlled numerical simulation module, Stage 7b) — producing research documents suitable for starting a theoretical economics working paper.
...
║                                                            ║
║    ⎇ github.com/wzj998/pAI-Econ-codex                ║
║                                                            ║
╟────────────────────────────────────────────────────────────╢
...
**First run:**
```
$pai-econ-codex "Your economic intuition, puzzle, or hypothesis."
```

**From a task file:**
```
$pai-econ-codex --task path/to/hypothesis.txt
```

**Resume after interruption:**
```
$pai-econ-codex --resume path/to/workspace
```
...
3. **Generate the manuscript PDF** by writing a complete LaTeX file and compiling with pdflatex:
   ...
   \author{%
     \texttt{pAI-Econ-codex} (\texttt{pAI-Econ-codex})\\[4pt]
     \small\url{https://github.com/wzj998/pAI-Econ-codex}\\[2pt]
     \small <ACTUAL MODEL NAME>%
   }
   \date{Draft: <Month DD, YYYY>}
   ```

   **⚠️ Model attribution:** Replace `<ACTUAL MODEL NAME>` with the name of the Codex model actually running this pipeline session (for example, the model name shown in the current Codex environment), as stated in your own system context. NEVER hardcode or copy a model name from an earlier project's manuscript — a paper generated by one model must not be attributed to another.
...
**pAI-Econ-codex** is developed by:
...
Repository: **pAI-Econ-codex** (MIT License)
