...
<p align="center">
  <b>一个连接经济学实证发现与理论建模的 Codex Skill</b><br>
  pAI-Econ-codex 并不试图替代理论经济学家的原创工作，而是帮助实证研究者把经验现象、机制直觉和实证发现转化为更清晰的理论问题、经典模型参照、可检验命题和可审计的研究框架。
</p>
...
本仓库相对于上游项目的改进应被理解为**Codex 适配与发布工程**，而不是对理论经济学 pipeline 原创性的重新主张：

- Codex skill 触发名与调用方式：`$pai-econ-codex`
- Codex app 元数据：`agents/openai.yaml`
- Codex plugin 包装：`plugins/pai-econ-codex/.codex-plugin/plugin.json`
- repo-local marketplace：`.agents/plugins/marketplace.json`
- 发布前检查：`scripts/release_check.py`
- demo hypothesis 短链路 forward test：`scripts/forward_test_demo.py`
- Windows / PowerShell UTF-8 使用规范，减少中文 Markdown 乱码误判
...
**pAI-Econ-codex** 是一个面向实证经济学家的 **human-in-the-loop Codex Skill**，旨在帮助研究者从经验现象、机制直觉和实证发现出发，补齐研究中的理论版图。
...
pAI-Econ-codex 试图为这些问题提供一个结构化工作流。它通过典范模型匹配、理论谱系检查、模型原语生成、假设审计、候选命题、证明草图和反例检查，帮助研究者把一个 empirical puzzle 转化为可讨论、可审查、可进一步合作发展的理论框架。
...
**pAI-Econ-codex 的定位是帮助研究者把理论建模过程显性化、文档化、可回溯化，并在关键节点保留人类判断。**
...
将本目录放入 Codex skills 目录后使用。项目显示名是 `pAI-Econ-codex`；Codex skill 触发名按规范使用小写：

```text
$pai-econ-codex "你的理论经济学研究想法"
```
...
这些检查会验证 Codex skill 元数据、plugin/marketplace 包装、核心目录结构、UTF-8 可读性、Claude 专属入口残留、生成工作区是否被误提交、本地 Git/远端状态，以及 demo hypothesis 是否能在根 skill 和 plugin-packaged skill 中走通短链路启动路径。Windows 下使用 `python -X utf8` 是为了避免中文 Markdown 被默认 GBK 解码而误报。

---

## Codex Plugin 与 Marketplace

本仓库同时提供 repo-local Codex plugin 包装：
...
plugin 内的 `skills/pai-econ-codex/` 是一个自包含 skill bundle，包含 `SKILL.md`、`prompts/`、`model_library/`、`templates/`、`docs/`、`examples/`、`NOTICE.md` 和 `LICENSE`。这样做是为了让 Codex plugin 安装后不依赖仓库根目录的相对路径。
...
pAI-Econ-codex 使用一个分阶段、可回溯的人机协作流程。
...
pAI-Econ-codex 内置一个 `model_library/`，用于在建模前先匹配经典理论模型家族。
...
├── CODEX.md                              # 项目级规则（文献验证要求、PDF 格式标准等）
├── README.md                             # 中文说明（本文件）
├── README_EN.md                          # 英文说明
├── THEORETICAL_ECON_MIGRATION_PLAN.md    # 原项目从 pAI/MSc 迁移到理论经济学的设计记录
├── LICENSE
├── agents/
