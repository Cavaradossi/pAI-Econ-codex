...
<p align="center">
  ...
  <img alt="Codex" src="https://img.shields.io/badge/Codex-skill-blueviolet">
...
<p align="center">
  <b>A Codex Skill for human-in-the-loop theoretical economics research</b><br>
  pAI-Econ-codex bridges empirical findings and theoretical modeling — it helps empirical
...
### What This Codex Version Adds

Relative to the upstream project, this repository's additions should be read as
**Codex adaptation and release engineering**, not as a new claim over the
original theoretical-economics pipeline:

- Codex skill trigger and invocation format: `$pai-econ-codex`
- Codex app metadata: `agents/openai.yaml`
- Codex plugin packaging: `plugins/pai-econ-codex/.codex-plugin/plugin.json`
- repo-local marketplace: `.agents/plugins/marketplace.json`
- release checks: `scripts/release_check.py`
- demo-hypothesis short-chain forward test: `scripts/forward_test_demo.py`
- Windows / PowerShell UTF-8 guidance to avoid false mojibake diagnoses for Chinese Markdown
...
**pAI-Econ-codex** is a **human-in-the-loop Codex Skill** designed for empirical economists who want to strengthen the theoretical foundations of their research.
...
**pAI-Econ-codex's role is to make the theory modeling process explicit, documented, and traceable, while preserving human judgment at critical decision points.**
...
```bash
git clone https://github.com/wzj998/pAI-Econ-codex.git
cd pAI-Econ-codex
```

Place this directory under your Codex skills directory. The project display name is `pAI-Econ-codex`; the Codex skill trigger is lowercase:

```text
$pai-econ-codex "Your theoretical economics research idea"
```
...
python -X utf8 scripts/forward_test_demo.py
```

These checks validate the Codex skill metadata, plugin/marketplace packaging,
core repository layout, UTF-8 readability, Claude-only entry-point residue,
generated workspace leakage, local Git/remote status, and whether the demo
hypothesis can enter the short-chain startup path from both the root skill and
the plugin-packaged skill. On Windows, `python -X utf8` avoids false failures
when Chinese Markdown would otherwise be decoded with the local legacy code
page.
...
This repository also includes repo-local Codex plugin packaging:

```text
.agents/plugins/marketplace.json
plugins/pai-econ-codex/
└── .codex-plugin/plugin.json
└── skills/pai-econ-codex/
```

The bundled `skills/pai-econ-codex/` directory is self-contained. It includes
`SKILL.md`, `prompts/`, `model_library/`, `templates/`, `docs/`, `examples/`,
`NOTICE.md`, and `LICENSE`, so the installed plugin does not depend on relative
...
pAI-Econ-codex uses a staged, traceable human-AI collaboration process.
...
pAI-Econ-codex includes a `model_library/` for matching canonical theoretical model families before formal modeling begins.
...
├── CODEX.md                              # Project-level rules (citation verification, PDF style standards)
